...
const MoviesList = ({ title, movieList, isTVShow, gptSearchList }) => {
    return (
        <div className={`relative md:mt-[2%] ${gptSearchList == true ? '' : 'bg-gradient-to-t from-black'} flex flex-col`}>
            ...
            <div className={`inline-grid w-full overflow-y-auto ${gptSearchList == true ? 'grid-flow-row md:grid-flow-col' : 'grid-flow-col pl-[5%]'}`}>
                {
